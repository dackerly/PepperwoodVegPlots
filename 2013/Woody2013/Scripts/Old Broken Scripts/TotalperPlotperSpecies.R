...
for (i in 1:50)
{
	...
	### This is the tricky part: match(TR[TR$Plot == i, "Species"], species) 
  #returns a vector of the indices of the species vector (which is in the same order as the species column in the data frame for the particular plot you are looking at in the all list) that match with the limited vector of species in the TR data frame. In this case, this bit returns this vector: c(1, 3, 11, 13). Meaning that, in the new data frame you are building, you want to go to those rows, and change the "Adult" column to be the entire column of "Number" in the TR data frame.
  #Using match allows everything to line up such that you can just pick a few different rows to deal with in 
  #"all" and set those few rows to the entire column of TR (for a given plot)
...
# should QUEGAR and QUEDOU be changed to QUEDEC? ok.Only use this code if you want to lump these.
# total[total$Species == "QUEGAR", "Species"]<-"QUEDEC"             
# total[total$Species == "QUEDOU", "Species"]<-"QUEDEC"     
# total[total$Species == "QUEKEL", "Species"]<-"QUEDEC"   
# x<-aggregate(total[total$Species =="QUEDEC","Adult"]~total[total$Species =="QUEDEC","Plot"], FUN=sum)[,2]             
# y<-aggregate(total[total$Species =="QUEDEC","Adult"]~total[total$Species =="QUEDEC","Plot"], FUN=sum)[,2]              
##############################             
...
for (i in 1:length(species)){
  cor$COR[i]<-cor((aggregate(total[total$Species==species[i],"Adult"]~total[total$Species == species[i],"Plot"], FUN=sum)[,2]), 
                  (aggregate(total[total$Species==species[i],"JS"]~total[total$Species ==species[i],"Plot"], FUN=sum)[,2]))
  
  
  cor$CORB[i]<-cor((aggregate(total[total$Species==species[i],"BA"]~total[total$Species == species[i],"Plot"], FUN=sum)[,2]), 
                   (aggregate(total[total$Species==species[i],"JS"]~total[total$Species ==species[i],"Plot"], FUN=sum)[,2]))  
             }           
...
# making scatterplots with TR basal area and SE+JU number
head(total)             
dim(total)
total.x<-subset
library("lattice")
xyplot(JS~Adult|Species, data=total)             
