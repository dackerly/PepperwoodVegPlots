...
#mega.data<-lapply(paste("/Users/meaganoldfather/Dropbox/PepperwoodVegPlots_2013 (2)/Veg Survey Data/Data Processing/Woody Veg CSV Files/Test/",list.files("/Users/meaganoldfather/Dropbox/PepperwoodVegPlots_2013 (2)/Veg Survey Data/Data Processing/Woody Veg CSV Files/Test/", pattern="*.csv"), sep=""), read.csv) #Reads all .csv files in a specified folder into one list of data.frames; theoretically it should ignore .xls files and only read files with the .csv extension
mega.data<-lapply(paste("/Users/meaganoldfather/Dropbox/PepperwoodVegPlots_2013 (2)/Veg Survey Data/Data Processing/Woody Veg CSV Files/",list.files("/Users/meaganoldfather/Dropbox/PepperwoodVegPlots_2013 (2)/Veg Survey Data/Data Processing/Woody Veg CSV Files/", pattern="*.csv"), sep=""), read.csv) #Reads all .csv files in a specified folder into one list of data.frames; theoretically it should ignore .xls files and only read files with the .csv extension
...
for (i in 1:length(mega.data)) #iterates through the whole list of data.frames corresponding to the number of plots worth of .csv files entered
{
	...
	trees[[i]]<-cbind(mega.data[[i]][mega.data[[i]]$Type=='TR'&mega.data[[i]]$"Dead/ Stump"!='S', 1:5], DBH=mega.data[[i]][mega.data[[i]]$Type=='TR'&mega.data[[i]]$"Dead/ Stump"!='S', 11]) #Combines the desired columns into a new data.frame for trees; first part of cbind returns columns 1 through 5 and every row such that the "Type" column is equal to 'TR' and the "Dead/ Stump" column is NOT equal to 'D'; second part of cbind returns column 11 (the DBH column) and all rows such that the "Type" column is equal to 'TR' and the "Dead/ Stump" column is NOT equal to 'D'. Also names the DBH column as 'DBH'
	...
	names(tree.cover.by.plot[[i]])<-c("Plot", "Species", "Sum DBH (cm)")
	...
	if(length(tree.species)>0)
	{
		...
		{	
			tree.cover.by.plot[[i]]$"Sum DBH (cm)"[j]<-sum(as.numeric(trees[[i]][trees[[i]]$Species==tree.species[j],6])) #iterates through tree species, summing their DBH's and adding a column to the tree.cover.by.plot data.frame
		}
	}
	saplings[[i]]<-cbind(mega.data[[i]][mega.data[[i]]$Type=='SA'&mega.data[[i]]$"Dead/ Stump"!='D', 1:5], mega.data[[i]][mega.data[[i]]$Type=='SA'&mega.data[[i]]$"Dead/ Stump"!='D', 9:10]) #Same idea for the saplings; First part of cbind returns all rows such that "Type" column equals 'SA' AND such that the "Dead/ Stump" column is NOT 'D' but only the first 4 columns; second part of cbind returns all rows such that "Type" column equals 'SA' but only the 8th and 9th columns
	...
	sapling.cover.by.plot[[i]]<-as.data.frame(matrix(NA, nrow=length(sapling.species), ncol=4))

	names(sapling.cover.by.plot[[i]])<-c("Plot", "Species", "Sum SA BD (cm)", "Sum SA Branches")
...
write.csv(tree.cover, file="/Users/meaganoldfather/Dropbox/PepperwoodVegPlots_2013 (2)/Veg Survey Data/Data Processing/Tree_Cover.csv")
write.csv(sapling.cover, file="/Users/meaganoldfather/Dropbox/PepperwoodVegPlots_2013 (2)/Veg Survey Data/Data Processing/Sapling_Cover.csv" )