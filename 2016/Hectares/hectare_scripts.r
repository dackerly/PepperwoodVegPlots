...
applySubplots <- function(treepoints, size, bbox=FALSE, plot_sub=FALSE) { 
  ...
  coords <- read.csv('https://raw.githubusercontent.com/dackerly/PepperwoodVegPlots/master/2016/Hectares/hectare_corners.csv',header=TRUE,sep=",") 
...
getHectareTrees <- function(na.omit=TRUE, basal.area = TRUE, agg.trunks=TRUE, filter.size=TRUE, trim.data=TRUE) { 
  ...
  get.trees <- read.csv('https://raw.githubusercontent.com/dackerly/PepperwoodVegPlots/master/2016/Hectares/hectare_trees_all.csv',header=TRUE,sep=",") 
