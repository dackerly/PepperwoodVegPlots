...
file.list <- dir(paste(getwd(),"/2013/Woody2013/Data/OriginalCSV/PlotInfo/", sep=''))
...
plot.data<-lapply(paste("2013/Woody2013/Data/OriginalCSV/PlotInfo/", file.list,sep=''), read.csv, skip=5, nrows=5, header=F)
...
options(stringsAsFactors=FALSE) 
plot.list<-get.plot()
file.dir <-paste(getwd(),"/", year, "/Woody",year, "/Data/OriginalCSV/PlotInfo/", sep='')
files<-dir(file.dir)
if(file.exists(paste(file.dir,files[1], sep=""))==F) stop("ERROR! Hold your horses! We don't yet have environemental surveys for this year")
envr.data<-lapply(paste(file.dir,files, sep=''), read.csv, skip=16, header=T)
...
if(!is.numeric(year)) stop("ERROR! Year must be numeric, try again cowboy")
 files<-paste(year, "/Mortality", year, "/Dead_Inds.csv" ,sep="")
...
options(stringsAsFactors=FALSE)  
file.list <-dir(paste(getwd(),"/2013/Woody2013/Data/OriginalCSV/Woody/", sep='')) 
plot.list<-get.plot()

mega.data<-lapply(paste("2013/Woody2013/Data/OriginalCSV/Woody/", file.list,sep=''), read.csv, na.strings=c("","NA") , skip=3) 
...
AUG.ID<-read.csv("2013/OakID2013/Aug_Species.csv")
...
species<-unique(indv.data$Species)
...
get.seju.data<-function(year){
  plot.list<-get.plot()
  file.dir <-paste(getwd(),"/", year, "/Woody",year, "/Data/OriginalCSV/Seedling/", sep='')
  files<-dir(file.dir)
  ...
  seju<-lapply(paste(file.dir,files,sep=''), read.csv,na.strings="", skip=3, header=T)
