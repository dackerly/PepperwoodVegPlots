...
### get.indv.data.2018() ### 
######################################################################
get.indv.data.2018<-function(year=2018, stump=F,orig.dead=F, branches=F,prefix='https://raw.githubusercontent.com/dackerly/PepperwoodVegPlots/master/'){
  options(stringsAsFactors=FALSE)  
  file.list <-(paste(prefix,"2018/EnteredData2018/WoodySurvey2018/CSV/WoodySurvey2018_PPW", sep='')) 
  plot.list<-get.plot()
  plot.list<-c("1301","1302","1303","1304","1305","1306","1307","1308","1309","1310","1311","1313","1314","1315","1316","1317","1318","1322","1323","1324","1325","1329","1331","1332","1334","1336")
  #plot.list<-c("1301","1302","1303","1304","1305","1306","1307","1308","1309","1310","1311","1313","1314","1315","1316","1317","1318","1319","1322","1323","1324","1325","1329","1331","1332","1334","1336","1338","1339","1340","1341","1342","1348","1349","1350","1851","1852","1853","1854")
  
  mega.data<-lapply(paste(file.list, plot.list, ".csv", sep=''), function(x) read.csv(text=getURL(x, followlocation = TRUE, cainfo = system.file("CurlSSL", "cacert.pem", package = "RCurl")), na.strings=c("","NA") , skip=3)) 
  #i=4
 # x <- paste(file.list,plot.list,'.csv',sep='')
  #mega.data=list()
  #mega.data[[1]]=read.csv(text=getURL(x, followlocation = TRUE, cainfo = system.file("CurlSSL", "cacert.pem", package = "RCurl")), na.strings=c("","NA") , skip=3)
  head(mega.data[[1]])
  length(mega.data)
  names(mega.data) <- plot.list
  ...
  for (i in 1:length(mega.data)) 
  {
    Plot<-plot.list[i]
    mega.data[[i]]<-cbind(Plot=Plot, mega.data[[i]]) 
    colnames(mega.data[[i]])<-c("Plot", "Quad", "Type", "Num", "X_cm", "Y_cm","Species",                             
                                "Survival","Basal","Epicormic","Apical","PercLivingCanopy","SA.Height_cm",
                                "SA.BD_cm","SA.Stem.Num","DBH_cm","Basal.Resprout.Height_cm","Basal.Resprout.Count","Tag.pulled","Notes") 
    mega.data[[i]]<-mega.data[[i]][,1:20] 
  }
  ...
  indv.data[grep("CERBET",indv.data$Species),"Species"]<-"CERBET"
  indv.data[grep("ARCMAN",indv.data$Species),"Species"]<-"ARCMAN"
  indv.data[grep("PIEMEN",indv.data$Species),"Species"]<-"PSEMEN"
  indv.data[grep("UMCAL",indv.data$Species),"Species"]<-"UMBCAL"
  ...
  # condense each individual into a single row 
  ##### NOTE 999'S summed here by species, not tracked by individual #####
  if (branches==F){
    indv.data$Num<-floor(indv.data$Num)
    ...
    indv.data<-data.table(indv.data)
    indv.data[,Basal.Area:=sum(Basal.Area),by="Num"]
    indv.data[,Survival:=max(Survival),by="Num"]
    indv.data[,Basal:=max(Basal),by="Num"]
    indv.data[,Apical:=max(Apical),by="Num"]
    indv.data[,Epicormic:=max(Epicormic),by="Num"]
    indv.data[,PercLivingCanopy:=max(PercLivingCanopy),by="Num"] #### BIT OF A FUDGE
    ...
    # Get rid of the columns "Basal.Area",... 
    indv.data<-as.data.frame(indv.data)
    indv.data<-indv.data[,c("Year","Plot","Quad","Type","Num","Species","X_cm","Y_cm",
                            "Survival","Basal","Epicormic","Apical","PercLivingCanopy","Basal.Area")]
